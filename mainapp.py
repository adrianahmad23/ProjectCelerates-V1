...
• Analisis kepribadian via Anthropic LLM
...
def download_models():
    """Download .pkl dari Google Drive kalau belum ada."""
...
def predict_rumpun(nilai_dict, peminatan, riasec_scores):
    ...
    vals = [float(nilai_dict.get(k, 70)) for k in MAPEL_ALL]
...
Tulis dalam bahasa Indonesia yang hangat, personal, dan menyemangati. Format:

**Kamu termasuk tipe {code}**
*{' - '.join(names[c] for c in top)}*

[Paragraf pembuka 2-3 kalimat yang menyambut dan memuji karakter mereka]

Lalu untuk setiap dimensi dominan ({top[0]}, {top[1]}, {top[2]}), tulis masing-masing:
**{names[top[0]]} ({top[0]})**
[2-3 paragraf deskripsi mendalam tentang kepribadian, cara belajar, lingkungan ideal]

Akhiri dengan 1 paragraf motivasi singkat yang menghubungkan ketiga dimensi dan mendorong mereka untuk percaya diri memilih jurusan.

Jangan gunakan poin/bullet. Gunakan paragraf mengalir. Total panjang 400-500 kata."""
    ...
    try:
        ...
        yield f"\n\nAPI Error: {e}\n\n" + _fallback_analysis(nama, code, scores)
...
with st.sidebar:
    ...
    st.markdown(
        ...
        '<b>LLM:</b> Claude Sonnet'
        '</div>', unsafe_allow_html=True)

    if not MODEL_READY:
        st.error(f"Model belum ada.\nJalankan `train_rumpun.py` dulu.")
...
if st.session_state.step == 1:
    ...
    with c2:
        ...
        for i, mp in enumerate(mpl_show):
            with cc[i % 2]:
                nilai_dict[mp] = st.number_input(
                    MAPEL_LABEL[mp], 0, 100,
                    int(st.session_state.nilai.get(mp, 75)), 1, key=f"n_{mp}")
        for mp in MAPEL_ALL:
            if mp not in nilai_dict:
                nilai_dict[mp] = st.session_state.nilai.get(mp, 70)

    # Metrics
    st.markdown("---")
    vals  = list(nilai_dict.values())
...
# ══════════════════════════════════════════════════════════════
    ...
    st.progress(n_answered / 42, text=f"Terjawab: {n_answered} / 42")
    ...
    with b2:
        if st.button("Lihat Hasil →", use_container_width=True, type="primary"):
            n_ans = len([v for v in answers.values() if v is not None])
            if n_ans < 21:
                st.warning(f" Baru {n_ans}/42 pertanyaan dijawab. Jawab minimal 21 untuk hasil yang akurat.")
            else:
...
# ══════════════════════════════════════════════════════════════
    ...
    tab1, tab2, tab3 = st.tabs(
        ["Analisis Kepribadian (RIASEC)", "Rekomendasi Rumpun (ML)", "SHAP & Detail"])
    ...
    with tab1:
        ...
        if ANTHROPIC_OK and api_key:
            ...
            st.markdown(
                ...
                '<b>Analisis personal</b> dihasilkan oleh Claude AI berdasarkan profil RIASEC kamu'
                '</div>', unsafe_allow_html=True)

            analysis_placeholder = st.empty()
            full_text = ""

            with st.spinner("Claude sedang menganalisis kepribadianmu..."):
                for chunk in generate_riasec_analysis_stream(nama, code, scores, api_key):
                    full_text += chunk
                    # Render markdown dalam bubble
                    analysis_placeholder.markdown(
                        f'<div class="chat-bubble">{full_text.replace(chr(10),"<br>")}</div>',
                        unsafe_allow_html=True)
        else:
            # Static fallback — tampilkan per dimensi
            st.markdown(
            ...
            if not api_key:
                st.info("Isi **Anthropic API Key** di sidebar untuk mendapatkan analisis personal yang lebih mendalam dari Claude AI.")
    ...
    with tab2:
        if not MODEL_READY:
            ...
            vals_n = [st.session_state.nilai.get(k, 70) for k in MAPEL_ALL]
    ...
    with tab3:
        if shap_df_dict:
            shap_df = pd.DataFrame(shap_df_dict)
            st.markdown(f"#### Faktor Penentu AI — Rumpun: {hasil_ml[0]['rumpun'] if hasil_ml else ''}")
            shap_df["arah"] = shap_df["kontribusi"].apply(lambda x: "Mendukung" if x > 0 else "Mengurangi")
            fig_s = px.bar(
                shap_df.sort_values("kontribusi", key=abs, ascending=True).tail(10),
                x="kontribusi", y="fitur", orientation="h", color="arah",
                color_discrete_map={"Mendukung":"#4ade80","Mengurangi":"#f87171"},
                title="Kontribusi Setiap Fitur (SHAP Explanation)",
                labels={"kontribusi":"Nilai SHAP","fitur":"Fitur"})
            fig_s.add_vline(x=0, line_dash="dash", line_color="#94a3b8")
            fig_s.update_layout(height=420,
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
            st.plotly_chart(fig_s, use_container_width=True)

            pos = shap_df[shap_df["kontribusi"]>0].head(3)
            neg = shap_df[shap_df["kontribusi"]<0].head(2)
            if not pos.empty:
                st.success("Faktor utama yang mendukung: " +
                           ", ".join(f"`{r.fitur}` (+{r.kontribusi:.3f})"
                                     for _,r in pos.iterrows()))
            if not neg.empty:
                st.info("Perlu ditingkatkan: " +
                        ", ".join(f"`{r.fitur}` ({r.kontribusi:.3f})"
                                  for _,r in neg.iterrows()))
        else:
            st.info("Jalankan model terlebih dahulu untuk melihat SHAP explanation.")
        ...
        with c1i:
            ...
            for k in MAPEL_ALL:
                ...
                st.markdown(
                    ...
                    f'<span style="width:100px;font-size:0.8rem">{MAPEL_LABEL[k].split(" ",1)[-1]}</span>'
